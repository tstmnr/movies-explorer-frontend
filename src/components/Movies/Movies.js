...
function Movies({ moviesList, setMoviesList, onCardLike, searchMovies, savedMoviesList }) {
  ...
  useEffect(() => {
    if (JSON.parse(localStorage.getItem('movies')) !== null && JSON.parse(localStorage.getItem('movies')).length > 0) {
      ...
      console.log(JSON.parse(localStorage.getItem('moviesSearchQuery')));
      setMoviesList(filterMoviesByKeyword(JSON.parse(localStorage.getItem('movies'))), JSON.parse(localStorage.getItem('moviesSearchQuery')), isChecked);
    }
  }, [isChecked]);
